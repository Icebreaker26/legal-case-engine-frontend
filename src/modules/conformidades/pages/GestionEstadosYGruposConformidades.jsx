...
export default function GestionEstadosYGruposConformidades() {
    ...
    const fetchDatos = async () => {
        try {
            const [est, gru, ent, proy, cont] = await Promise.all([
            ...
            setDatos({ estados: est.data, grupos: gru.data, entidades: ent.data, proyectos: proy.data, contratos: cont.data });
    ...
    const handleCrear = async (tipo, nombre) => {
        ...
        try {
            ...
            else if(tipo === 'grupos') await apiService.post(`/conformidades/grupos`, { nombre });
    ...
    const iniciarEdicion = (tipo, id, nombre) => setEditando({ id, tipo, valor: nombre });

    const guardarEdicion = async () => {
        if (!editando.valor.trim()) return;
        try {
            const endpointBase = editando.tipo === 'contratos' ? 'contratos' : editando.tipo;
            await apiService.patch(`/conformidades/${endpointBase}/${editando.id}`, 
                editando.tipo === 'contratos' ? { numero: editando.valor } : { nombre: editando.valor }
            );
    ...
    const handleEliminar = async (tipo, id) => {
        ...
        try {
            let endpoint = tipo;
            await apiService.delete(`/conformidades/${endpoint}/${id}`);
