...
import MainTabs from '../components/detalle/MainTabs';

// Modals
import ModalDocumentoCompleto from '../components/detalle/modals/ModalDocumentoCompleto';
import ModalEliminarTutela from '../components/detalle/modals/ModalEliminarTutela';
import ModalBorradorIA from '../components/detalle/modals/ModalBorradorIA';
import { ModalCrearRequerimiento, ModalVerOficio, ModalResponderReq } from '../components/detalle/modals/ModalRequerimientos';
...
export default function DetalleTutela() {
  ...
  const handleVerDocumentoCompleto = async (sug) => {
    ...
    try {
        const data = await tutelaService.obtenerDocumentoReferencia(sug.documento_id, sug.contenido_legal);
  ...
  const handleGenerarIA = async () => {
    ...
    setLoadingAi(true);
    try {
      const result = await tutelaService.obtenerSugerenciasBorrador(id);
      // Si hay borrador guardado lo carga, si no deja el editor vacío con las sugerencias
      setAiDraftContent(result.borrador_completo || '');
      const success = await lock();
      if (!success) return;

      setAiDraftModalOpen(true);
      toast.success(result.status === 'cached' ? 'Borrador guardado cargado' : 'Precedentes cargados — redacta el borrador');
    } catch (error) {
